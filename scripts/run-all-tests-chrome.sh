...
echo "Restarting Selenium2 with standalone version 3 and chrome browser..."
sudo ./scripts/stop-selenium.sh
./scripts/start-selenium-chrome.sh &> /dev/null
...
do
    ...
    ./bin/behat --config=app/config/behat.yml --suite="${suite}" -p chrome
done
